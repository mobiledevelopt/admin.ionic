...
export class ApisService {
  ...
  uploadQris(files: File[]) {
    const formData = new FormData();
    Array.from(files).forEach((f) => formData.append("userfile", f));
    return this.http.post(this.baseUrl + "users/upload_image_qris", formData);
  }
  ...
  public auth(body): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      ...
      const param = this.JSON_to_URLEncoded(body);
      console.log(param);
      this.http.post(this.baseUrl + "users/getById", param, header).subscribe(
        (data: any) => {
          console.log(data);
