...
export class ManageAreaComponent implements OnInit {
  ...
  create() {
    ...
    if (!this.city || this.city === '') {
      this.error(this.api.translate('Please select valid Area name'));
      return false;
    }
    const param = {
      name: this.city,
    ...
    this.spinner.show();
    this.api.post('area/save', param).then(data => {
      this.spinner.hide();
      console.log(data);
      this.api.alerts(this.api.translate('Success'), this.api.translate('Area Added'), 'success');
      this.navCtrl.back();
    }, error => {
      this.spinner.hide();
      console.log(error);
